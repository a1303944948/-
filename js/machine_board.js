...
$.ajax({ 
	type: 'get',
	url: URLZ + '/store/'+ loginUserName.empcode +'/level',
	async: false,
	success: function(data){
		...
		for(var i = 0; i <　data.resultObject.length; i++){
			if(Object.is(data.resultObject[i].icon,'1')){
				var BOARDSELECTBOBJ = {};
				BOARDSELECTBOBJ.type = data.resultObject[i].operatorID;
				BOARDSELECTBOBJ.text = data.resultObject[i].text;
...
userHeadSubmit.onclick = function(){
	...
	var	userHeadGroup = loginUserName.scopeofauthority;
...
function submit(){
	...
	bodySubmit.onclick = function(){
		...
		userObject.cooperationMode = '';
		// var date = new Date();
		// if(boardConcur.value == "购买"){
		userObject.buyTime = '';
		userObject.leaseStartTime = '';
		userObject.leaseExpireTime = '';
		// }else{
